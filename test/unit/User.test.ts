...
test("Não deve criar um usuário com nome inválido", function () {
  expect(() => User.create("John", "john.doe@gmail.com", "12345678", 30)
  ).rejects.toThrow(new Error("Invalid name"))
})

test("Não deve criar um usuário com e-mail inválido", function () {
  expect(() => User.create("John Doe", "john.doe@gmail", "12345678", 30)
  ).rejects.toThrow(new Error("Invalid email"))
})

test("Não deve criar um usuário com senha inválido", function () {
  expect(() => User.create("John Doe", "john.doe@gmail.com", "123456", 30)
  ).rejects.toThrow(new Error("Invalid password"))
})

test("Não deve criar um usuário com idade inválido", function () {
  expect(() => User.create("John Doe", "john.doe@gmail.com", "12345678", 17)
